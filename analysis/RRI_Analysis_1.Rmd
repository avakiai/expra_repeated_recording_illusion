...
for (fileN in unique(as.character(data_breakdown$ID))) {
  ...
  # dependent variables
  this_maindata <- cbind(this_data$questions, this_data$ratings_slider.response, this_data$open_text_response.text, this_data$eval_slider.response)[which(this_data$questions!=""),]
  ...
  this_data3 <- this_data2 %>%
    setNames(c("condition", "genre", "prime_order", "probe", "question", "rating", "open_text", "final_eval")) %>%
