...
rating_summary <- rating_data %>% group_by(wm, condition, question_n) %>% summarise(mean_rating = ???,
                                                                sd_rating = ???,
                                                                se_rating = sd_rating/sqrt(nrow(rating_data)))

# average ratings over questions
ratings_mu_summary <- rating_data %>% group_by(???) %>% summarise(???)
```
...
ggplot(rating_summary) + 
  ...
  geom_errorbar(aes(x = question_n, y = mean_rating, # add errorbars, given specs on ymin/ymax
                   ymin = mean_rating-???, ymax = mean_rating+???, color = condition),
...
```{r message=FALSE, warning=FALSE}
genre_summary <- rating_data %>% group_by(wm, condition, genre, question_n) %>% summarise(???)

genre_mu_summary <- rating_data %>% group_by(wm, condition, genre) %>% summarise(???)
```
...
class_olmm <- clmm(mean_rating ~ ??? + (1|ID), 
                   data = data_class) 
...
# odds ratio & ci
exp(cbind(OR = ???(class_olmm), ???(class_olmm)))
```
